...
object ErrorCategory {
  ...
  val all: Seq[ErrorCategory] =
    Seq(
      ...
      IsAbortShouldBePrecondition,
      IsPermissionDeniedShouldBeUnavailable,
    )
  ...
  /** Service is temporarily unavailable
    ...
    *
    * Resolution: Expectation: transient failure that should be handled by retrying the request
  ...
  // TODO(i5990) remove. currently needed as otherwise ledger-api server translates UNAVAILABLE to INTERNAL
  @Deprecated
  object IsPermissionDeniedShouldBeUnavailable
      extends ErrorCategoryImpl(
  ...
  /** Failure due to contention on some resources
    *
    * Description: The request could not be processed due to shared processing resources
  ...
  /** Request completion not observed within a pre-defined window
    *
    * NOTE: This is not a request failure. Just the notification that the completion
    * was not observed within a pre-defined timeframe.
    *
    ...
    *
    * Resolution: Expectation: the deadline might have been exceeded due to transient resource
  ...
  /** Some internal error
    ...
    *
    * Resolution: Expectation: this is due to a bug in the implementation or data corruption in the systems databases.
  ...
  /** Malicious or faulty behaviour detected
    ...
    * Retry strategy: retry after operator intervention
    *
    * Resolution: Expectation: this can be a severe issue that requires operator attention or intervention, and
  ...
  /** Client is not authenticated properly
    ...
    *
    * Resolution: Expectation: this is an application bug, application misconfiguration or ledger-level
  ...
  /** Client does not have appropriate permissions
    ...
    * Retry strategy: retry after app operator intervention
    *
    * Resolution: Expectation: this is an application bug or application misconfiguration. Resolution requires
  ...
  /** A request which is never going to be valid
    ...
    * Retry strategy: retry after app operator intervention
    *
    * Resolution: Expectation: this is an application bug or ledger-level misconfiguration (e.g. request size limits).
  ...
  /** A failure due to a resource already existing in the current system state
    ...
    *              resource.
    *
    * Retry strategy: inspect resource failure and retry after resource failure has been resolved (depends on type of
