...
object TypedValueGenerators {
  ...
  object ValueAddend extends PrimInstances[Lambda[a => ValueAddend { type Inj = a }]] {
    ...
    val contractId: Aux[ContractId] = new ValueAddend {
      ...
      override def injarb = Arbitrary(ValueGenerators.coidGen)
      override def injshrink = implicitly[Shrink[ContractId]]
    }
    ...
    def map(elt: ValueAddend): Aux[SortedLookupList[elt.Inj]] = new ValueAddend {
      ...
      override def injshrink = Shrink.shrinkAny // XXX descend
    }
    ...
    def enum(
      ...
      (
        ...
        new EnumAddend[members.type] {
          ...
          override def injarb = Arbitrary(Gen.oneOf(values))
          override def injshrink =
            Shrink { ev =>
  ...
  def genTypeAndValue: Gen[(Type, Value)] =
    for {
      addend <- genAddend
